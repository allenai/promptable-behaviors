...
class ProcTHORFleeNavRGBClipResNet50PPOMORLExperimentConfig(ProcTHORFleeNavBaseConfig):
    """An Flee Navigation experiment configuration with RGB input."""
    ...
    def __init__(self, cfg, **kwargs):
        ...
        if not self.adaptive_reward:
            if cfg.morl.reward_embed_type == "codebook" or cfg.morl.reward_embed_type == "raw":
            ...
            elif cfg.morl.reward_embed_type == "integer":
                if self.reward_weights is None:
                    self.reward_weights = [1] * self.num_objectives
                cfg.morl.reward_weights = [int(x) for x in self.reward_weights]
    ...
    @classmethod
    def tag(cls):
        return "FleeNav-RGB-ClipResNet50GRU-DDPPO"
