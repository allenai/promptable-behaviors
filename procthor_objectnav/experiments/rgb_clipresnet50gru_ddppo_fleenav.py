...
from .base import ProcTHORFleeNavBaseConfig
from allenact.utils.experiment_utils import Builder


class ProcTHORFleeNavRGBClipResNet50PPOExperimentConfig(ProcTHORFleeNavBaseConfig):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        # MORL
        self.objectives_combination = "default"
        self.objectives = ["step_penalty", "path_efficiency", "goal"]
        self.valid_objectives = ["step_penalty", "path_efficiency", "goal"]
        self.num_objectives = len(self.valid_objectives)
        self.adaptive_reward = False
        self.reward_weights = [1.0, 1.0, 1.0] # no normalization
        cfg.morl.objectives_combination = self.objectives_combination
        cfg.morl.objectives = self.objectives
        cfg.morl.valid_objectives = self.valid_objectives
        cfg.morl.num_objectives = self.num_objectives
        cfg.morl.adaptive_reward = self.adaptive_reward
        cfg.morl.reward_weights = self.reward_weights
    ...
    def create_model(self, **kwargs) -> nn.Module:
        ...
        model = self.MODEL(
            ...
            use_reward_info=False, # baseline
        )
