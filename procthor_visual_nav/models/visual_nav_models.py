...
class VisualNavActorCritic(ActorCriticModel[CategoricalDistr]):
    ...
    def forward(  # type:ignore
        ...
        if self.use_reward_info:
            if self.reward_embed_type == "codebook" or self.reward_embed_type == "integer":
                ...
                reward_weights_embeds = observations['reward_weights'].type(torch.float32)
                reward_weights_embeds = reward_weights_embeds.reshape(reward_weights_embeds.shape[0], reward_weights_embeds.shape[1], -1)
        ...
        for key, model in self.state_encoders.items():
            # if memory has attribute tensor
            if hasattr(memory, "tensor"):
                ...
                # dictionary type memory
                beliefs_dict[key], rnn_hidden_states = model(
                    joint_embeds, memory[key], masks
                )
                memory[key] = rnn_hidden_states  # update memory here
