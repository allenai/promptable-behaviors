...
from procthor_visual_nav.models.visual_nav_models import (
    VisualNavActorCritic,
    FusionType,
)
# from projects.rlmf_models.reward_model import RewardModel
...
class ResnetTensorNavActorCritic(VisualNavActorCritic):
    def __init__(
        ...
        **kwargs,
        ...
        self.reward_embed_type = reward_embed_type
        if use_reward_info:
            if self.reward_embed_type == "codebook":
            ...
            elif self.reward_embed_type == "raw":
                reward_info_embed_size = 1 # per objective
            elif self.reward_embed_type == "integer":
                self.create_reward_weights_embedder(num_objectives=num_objectives, embed_size=action_embed_size*2, mode="integer")
                reward_info_embed_size = 2*action_embed_size
        else:
    ...
    def create_reward_weights_embedder(self, num_objectives: int, embed_size: int, mode='codebook'):
        if mode == 'codebook':
            ...
            self.reward_weights_embedder = FeatureEmbedding(input_size=30, output_size=embed_size) # prev_action_embed_size*2
        elif mode == 'raw':
...
class ResnetTensorGoalEncoder(nn.Module):
    ...
    def adapt_input(self, observations):
        ...
        # elif len(resnet.shape) == 4:
        #     nstep = 1
        #     nsampler = resnet.shape[0]
