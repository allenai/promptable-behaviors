...
from procthor_visual_nav.algorithms.onpolicy_engine import (
    TEST_MODE_STR,
    TRAIN_MODE_STR,
    VALID_MODE_STR,
    OnPolicyInference,
...
class OnPolicyRunner(object):
    ...
    def setup_callback_classes(self, callbacks: Optional[str]) -> Set[Callback]:
        ...
        for filename in files:
            ...
            for mod_class in classes:
                if not self.config.use_wandb and mod_class[0] == 'WandbLogging':
                    continue
    ...
    def start_test(
        ...
        metrics_dir = self.metric_path(self.local_start_time_str)
        if self.extra_tag != "":
            metrics_dir += "_" + self.extra_tag
        reward_weights_string = "_".join(["{:.3f}".format(w) for w in self.config.reward_weights])
        metrics_tag = "_weights{}".format(reward_weights_string)
        metrics_dir = metrics_dir + metrics_tag
        os.makedirs(metrics_dir, exist_ok=True)
    ...
    def process_train_packages(
        ...
        for k in sorted(
            ...
            message.append(f"{short_key} {means[k]:.3f}")
        message += [f"elapsed_time {(current_time - last_time):.3f}s"]

        if last_steps > 0:
            fps = (training_steps - last_steps) / (current_time - last_time)
            message += [f"approx_fps {fps:.3f}"]
        ...
        for (
            ...
            if storage_uuid in storage_uuid_to_total_experiences:
                ...
                message += [f"{storage_uuid}/approx_eps {eps:.3f}"]
    ...
    def process_test_packages(
        ...
        for k in sorted(metric_means.keys()):
            ...
            if log_writer is not None:
                log_writer.add_scalar(metrics_key, metric_means[k], training_steps)
            callback_metric_means[metrics_key] = metric_means[k]
            message.append(k + f" {metric_means[k]:.3f}")
