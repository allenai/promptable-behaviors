...
from procthor_visual_nav.utils.debug_utils import bcolors
from procthor_visual_nav.utils.video_utils import save_video
...
from procthor_visual_nav.utils.experiment_utils import (
    LoggingPackage,
    PipelineStage,
    ScalarMeanTracker,
    StageComponent,
    ...
    def vector_tasks(
        ...
        if self._vector_tasks is None and self.num_samplers > 0:
            ...
            self._vector_tasks = VectorSampledTasks(
                ...
                read_timeout=3600, #DEBUG_VST_TIMEOUT if DEBUGGING else 1 * 60,
            )
...
class OnPolicyInference(OnPolicyRLEngine):
    def __init__(
        ...
        **kwargs,
        ...
        video_folder_name = datetime
        if not config.adaptive_reward:
            self.reward_weights_string = "_".join(["{:.3f}".format(weight) for weight in config.reward_weights])
        else:
            self.reward_weights_string = "adaptive"
        video_folder_name = datetime+"_"+config.extra_tag+("_weights"+self.reward_weights_string)
        self.video_dir = "./storage/objectnav-robothor-rgb-clip-rn50/videos/{}".format(video_folder_name)
        if (self.mode != "train") and (not os.path.exists(self.video_dir)):
            os.makedirs(self.video_dir)

        self.env_name = self.config.__class__.__name__.lower()
        if "objectnav" in self.env_name:
            self.env_name = "objectnav"
        elif "fleenav" in self.env_name:
            self.env_name = "fleenav"
        date = time.strftime("%m%d")
        self.trajectory_dir = "./storage/trajectories-{}-{}".format(self.env_name, date)
        
        self.sub_rewards_dict = None
    ...
    def run_eval(
        ...
        while self.num_active_samplers > 0:
            if (self.config.save_trajectory or self.config.save_video) and num_logged > 2000:
                # reset num_logged and logging_pkg and steps to avoid memory error
                num_logged = 0
                logging_pkg = LoggingPackage(
                    mode=self.mode,
                    training_steps=total_steps,
            ...
            if self.num_active_samplers == 0 or cur_time - last_time >= update_secs:
                ...
                if len(logging_pkg.metric_dicts) > num_logged and self.config.save_video:
                    for i in range(num_logged, len(logging_pkg.metric_dicts)):
                        rgb_frames = logging_pkg.metric_dicts[i]["rgb"] #self.episode_infos[0]["rgb"]
                        # topdown_frames = logging_pkg.metric_dicts[i]["topdown"]
                        # # concat rgb and topdown
                        # rgb_frames = np.concatenate((rgb_frames, topdown_frames), axis=1)
                        
                        episode_id = logging_pkg.metric_dicts[i]["task_info"]["id"]
                        if self.env_name == "objectnav":
                            success = int(logging_pkg.metric_dicts[i]["success"])
                        elif self.env_name == "fleenav":
                            success = int(logging_pkg.metric_dicts[i]["normalized_success"])
                        spl = logging_pkg.metric_dicts[i]["spl"]
                        
                        if self.env_name == "objectnav":
                            ##### ObjectNav #####
                            dist_to_target = logging_pkg.metric_dicts[i]["dist_to_target"]
                        elif self.env_name == "fleenav":
                            ##### FleeNav #####
                            dist_to_target = logging_pkg.metric_dicts[i]["dist_to_furthest_point"]
                        ep_length = logging_pkg.metric_dicts[i]["ep_length"]
                        reward = logging_pkg.metric_dicts[i]["reward"]
                        weights = logging_pkg.metric_dicts[i]["task_info"]["reward_weights"]
                        weights_string = "_".join(["{:.3f}".format(weight) for weight in weights])
                        sub_rewards = [logging_pkg.metric_dicts[i][key] for key in logging_pkg.metric_dicts[i] if "sub_reward/total" in key]
                        sub_rewards_string = "_".join(["{:.3f}".format(sub_reward) for sub_reward in sub_rewards])
                        
                        save_video(rgb_frames, os.path.join(self.video_dir, "{}_weights{}_sr{}_spl{:.3f}_dist{:.3f}_eplen{}_reward{:.3f}_subrewards{}.mp4".format(episode_id, weights_string, success, spl, dist_to_target, ep_length, reward, sub_rewards_string)))
                        print(bcolors.OKCYAN + "{}/{} Video Saved!!".format(num_logged, num_tasks) + bcolors.ENDC)
                    ...
                    if logging_pkg.num_non_empty_metrics_dicts_added != 0:
                        ...
                        if self.config.save_trajectory:
                            for i in range(num_logged, len(logging_pkg.metric_dicts)):
                                ...
                                sub_rewards = [logging_pkg.metric_dicts[i][key] for key in logging_pkg.metric_dicts[i] if "sub_reward/total" in key]
                                sub_rewards_string = "_".join(["{:.3f}".format(sub_reward) for sub_reward in sub_rewards])
                                logging_pkg.metric_dicts[i]["task_info"]["obs_embeds"] = np.stack(logging_pkg.metric_dicts[i]["task_info"]["obs_embeds"], axis=0)

                                # save logging_pkg.metric_dicts[i]
                                if not os.path.exists(self.trajectory_dir):
                                    os.makedirs(self.trajectory_dir)
                                joblib.dump(logging_pkg.metric_dicts[i], os.path.join(self.trajectory_dir, "{}_weights{}_sr{}_spl{:.3f}_dist{:.3f}_eplen{}_reward{:.3f}_subrewards{}.pkl".format(episode_id, weights_string, success, spl, dist_to_target, ep_length, reward, sub_rewards_string)), compress=3)
                                print(bcolors.OKCYAN + "{}/{} Trajectory Saved!!".format(num_logged, num_tasks) + bcolors.ENDC)

                        if self.config.save_video or self.config.save_trajectory:
                            num_logged = len(logging_pkg.metric_dicts)
