...
from procthor_visual_nav.utils.types import Vector3
...
def distance_to_object_type(
    ...
    def path_from_point_to_object_type(
        point: Dict[str, float], object_type: str, allowed_error: float
    ) -> Optional[List[Dict[str, float]]]:
        event = controller.step(
            action="GetShortestPath",
        ...
        if event:
            ...
            get_logger().debug(
                f"Failed to find path for {object_type} in {house_name}."
                f' Start point {point}, agent state {event.metadata["agent"]}.'
            )
            return None
    ...
    def distance_from_point_to_object_type(
        ...
        It might return -1.0 for unreachable targets.
        """
        path = path_from_point_to_object_type(point, object_type, allowed_error)
        if path:
            # Because `allowed_error != 0` means that the path returned above might not start
            # at `point`, we explicitly add any offset there is.
            dist = position_dist(p0=point, p1=path[0], ignore_y=True)
    ...
    def retry_dist(position: Dict[str, float], object_type: str) -> float:
        allowed_error = 0.05
        debug_log = ""
        d = -1.0
        while allowed_error < 2.5:
            d = distance_from_point_to_object_type(position, object_type, allowed_error)
            if d < 0:
                debug_log = (
                    f"In house {house_name}, could not find a path from {position} to {object_type} with"
                    f" {allowed_error} error tolerance. Increasing this tolerance to"
                    f" {2 * allowed_error} any trying again."
                )
                allowed_error *= 2
        ...
        if d < 0:
            get_logger().warning(
                f"In house {house_name}, could not find a path from {position} to {object_type}"
                f" with {allowed_error} error tolerance. Returning a distance of -1."
            )
        elif debug_log != "":
            get_logger().debug(debug_log)
