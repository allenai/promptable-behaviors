...
from procthor_visual_nav.tasks.object_nav import (
    FullProcTHORObjectNavTestTaskSampler,
    FullProcTHORObjectNavHideObjectsTestTaskSampler,
    FullProcTHORObjectNavSpawnObjectsTestTaskSampler,
    FullProcTHORObjectNavCreateObjectsAtLocationTestTaskSampler,
    ProcTHORObjectNavTask,
    ProcTHORObjectNavTaskSampler,
    ProcTHORObjectNavTestTaskSampler,
    ProcTHORProcTHORObjectNavTestTaskSampler,
)

from procthor_visual_nav.tasks.flee_nav import ProcTHORFleeNavTask, ProcTHORFleeNavTaskSampler, FullProcTHORFleeNavTestTaskSampler
...
from procthor_visual_nav.utils.types import RewardConfig, TaskSamplerArgs
from procthor_visual_nav.losses.KL_loss import KLLoss


class ProcTHORObjectNavBaseConfig(ExperimentConfig, ABC):
    """The base config for all ObjectNav experiments."""

    DISTANCE_TYPE = "geo"  #"l2"  #  Can be "geo" or "l2"
    ...
    def __init__(self, cfg, **kwargs):
        ...
        if self.EVALUATE and "robothor" in cfg.evaluation.tasks:
            if len(cfg.evaluation.tasks) == 1:
                self.THOR_COMMIT_ID: Optional[str] = "bad5bc2b250615cb766ffb45d455c211329af17e"
            else:
                raise NotImplementedError(
                    f"RoboTHOR evaluation should be evaluated by itself only with its own commit id"
                )
        else:
            self.THOR_COMMIT_ID: Optional[str] = "ca10d107fb46cb051dba99af484181fda9947a28"

        self.DISTANCE_TYPE = cfg.agent.distance_type

    def get_devices(self, split: Literal["train", "valid", "test"]) -> Tuple[torch.device]:
    ...
    def machine_params(self, mode: Literal["train", "valid", "test"], **kwargs):
        ...
        if mode == "train":
            ...
            nprocesses = self.cfg.machine.num_train_processes * self.cfg.distributed.nodes
        elif mode == "valid":
    ...
    def make_sampler_fn(
        ...
        task_sampler_args.controller_args["server_timeout"] = 60*60 #300
        # return ProcTHORObjectNavTaskMappingSampler(args=task_sampler_args, object_nav_task_type=self.OBJECT_NAV_TASK_TYPE)
    ...
    def _get_sampler_args_for_scene_split(
        ...
        # if "object_exploration" in self.cfg.morl.objectives:
        ...
        controller_args = {
            ...
            "renderInstanceSegmentation": False, #renderInstanceSegmentation,
            **self.get_platform(
        ...
        return TaskSamplerArgs(
            ...
            # morl
            objectives_combination=self.cfg.morl.objectives_combination,
            objectives=self.cfg.morl.objectives,
            valid_objectives=self.cfg.morl.valid_objectives,
            save_trajectory=self.cfg.evaluation.save_trajectory,
            save_video=self.cfg.evaluation.save_video,
            adaptive_reward=self.cfg.morl.adaptive_reward,
            reward_weights=self.cfg.morl.reward_weights,
    ...
    def train_task_sampler_args(self, **kwargs) -> Dict[str, Any]:
        ...
        if self.cfg.procthor.num_train_houses:
            train_houses = train_houses.select(range(self.cfg.procthor.num_train_houses))
    ...
    def valid_task_sampler_args(self, **kwargs) -> Dict[str, Any]:
        ...
        out = self._get_sampler_args_for_scene_split(
            # houses=val_houses.select(range(400)),
            houses=self.EVAL_TASKS["val"] if self.EVALUATE else val_houses.select(range(100)),
            mode="eval",
            allow_oversample=False,
            max_tasks=self.cfg.evaluation.max_val_tasks if self.EVALUATE else self.cfg.training.max_val_tasks,
    ...
    def test_task_sampler_args(self, **kwargs) -> Dict[str, Any]:
        ...
        out = self._get_sampler_args_for_scene_split(
            # houses=self.EVAL_TASKS["test"] if self.EVALUATE else test_houses.select(range(100)),
            houses=test_houses.select(np.arange(400)),
            mode="eval",
            allow_oversample=False,
            max_tasks=self.cfg.evaluation.max_test_tasks if self.EVALUATE else self.cfg.training.max_test_tasks,
            # max_tasks=200,
            allow_flipping=False,
            resample_same_scene_freq=self.RESAMPLE_SAME_SCENE_FREQ_IN_INFERENCE,
            # extra_controller_args=None if self.EVALUATE else dict(scene="Procedural"),
    ...
    def training_pipeline(self, **kwargs):
        ...
        for n in range(self.cfg.training.num_stages):
            ...
            if torch.cuda.is_available:
                log_intervals.append(self.cfg.distributed.nodes * self.cfg.machine.num_train_processes * num_steps[-1] * 5)
            else:
            ...
            if n < self.cfg.training.num_stages - 1:
                ...
                if len(batch_steps) > 0:
                    batch_steps.append(int(self.cfg.training.ppo_steps - sum(batch_steps)))
                else:
        ...
        if self.cfg.losses.KL_loss.alpha > 0:
            named_losses["KL_loss"] = KLLoss(alpha=self.cfg.losses.KL_loss.alpha)
            loss_names.append("KL_loss")
            ...
            pipeline_stages=[
                PipelineStage(
                    ...
                    training_settings=TrainingSettings(
                        num_steps=num_steps[n], metric_accumulate_interval=log_intervals[n]
                    ),
                ) for n in range(self.cfg.training.num_stages)
            ],
        )
...
class ProcTHORFleeNavBaseConfig(ExperimentConfig, ABC):
    """The base config for all FleeNav experiments."""
    ...
    FLEE_NAV_TASK_TYPE = ProcTHORFleeNavTask
    ...
    def make_sampler_fn(
        ...
        task_sampler_args.controller_args["server_timeout"] = 60*60 #300
        # return ProcTHORFleeNavTaskMappingSampler(args=task_sampler_args, flee_nav_task_type=self.FLEE_NAV_TASK_TYPE)
    
        if self.EVALUATE:
            return FullProcTHORFleeNavTestTaskSampler(args=task_sampler_args)
        else:
            return ProcTHORFleeNavTaskSampler(
                args=task_sampler_args, flee_nav_task_type=self.FLEE_NAV_TASK_TYPE
            )
